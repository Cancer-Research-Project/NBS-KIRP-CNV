...
def network_propagation(network, binary_matrix, alpha=0.7, symmetric_norm=False, verbose=True, **save_args):
    ...
    subgraphs = list(network.subgraph(c)
    ...
    binary_matrix_filt = np.array(
        binary_matrix.T.loc[subgraph_nodes].fillna(0).T)
    ...
    prop_data = fast_random_walk(
        alpha, binary_matrix_filt, subgraph_norm, prop_data_empty)
    ...
    for subgraph in subgraphs[1:]:
        ...
        binary_matrix_filt = np.array(
            binary_matrix.T.loc[subgraph_nodes].fillna(0).T)
        subgraph_norm = normalize_network(
            subgraph, symmetric_norm=symmetric_norm)
        prop_data = fast_random_walk(
            alpha, binary_matrix_filt, subgraph_norm, prop_data)
    # Return propagated result as dataframe
    prop_data_df = pd.DataFrame(
        data=prop_data[:, 1:], index=binary_matrix.index, columns=prop_data_node_order)
    ...
    if 'outdir' in save_args:
        if 'job_name' in save_args:
            if 'iteration_label' in save_args:
                ...
                save_path = save_args['outdir'] + \
                    str(save_args['job_name'])+'_prop.csv'
        else:
            if 'iteration_label' in save_args:
                save_path = save_args['outdir']+'prop_' + \
                    str(save_args['iteration_label'])+'.csv'
            else:
        ...
        pass
    if verbose:
        print('Network Propagation Complete:',
...
def network_kernel_propagation(network, network_kernel, binary_matrix, verbose=False, **save_args):
    ...
    # print(len(diff_col))
    # print(len(diff_col))
    binary_matrix_copy = pd.concat(
        [binary_matrix, pd.DataFrame(columns=diff_col)]).fillna(0)
    # print(len(binary_matrix_copy))
    # print(len(network_kernel))
    # common_nodelist = binary_matrix_copy[prop_]
    prop_data = np.dot(
        binary_matrix_copy[prop_nodelist], network_kernel.loc[prop_nodelist][prop_nodelist])
    # Get propagated results for remaining subgraphs
    for nodelist in subgraph_nodelists[1:]:
        ...
        binary_matrix_copy2 = pd.concat(
            [binary_matrix, pd.DataFrame(columns=diff_col2)]).fillna(0)
    ...
    # Return propagated result as dataframe
    prop_data_df = pd.DataFrame(
        data=prop_data, index=binary_matrix.index, columns=prop_nodelist)
    # Saving the propagation result
    if 'outdir' in save_args:
        if 'job_name' in save_args:
            if 'iteration_label' in save_args:
